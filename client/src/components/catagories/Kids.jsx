...
import { fetchKidsProducts } from "../../Slices/kidsSlice"; // ✅ Import API fetch action
...
const Kids = () => {
  ...
  const {
    ...
    loading,
    error,
  } = useSelector((state) => state.kids); // ✅ Use kids slice
  const favoriteItems = useSelector((state) => state.favorite.favorite);

  useEffect(() => {
    dispatch(fetchKidsProducts()); // ✅ Fetch Kids' products from API
  }, [dispatch]);
