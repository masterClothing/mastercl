...
import { fetchSaleProducts } from "../../Slices/saleSlice"; // ✅ Import the correct action for sale products
...
const Sale = () => {
  ...
  const {
    ...
    loading,
    error,
  } = useSelector((state) => state.sale); // ✅ Use sale slice
  const favoriteItems = useSelector((state) => state.favorite.favorite);

  useEffect(() => {
    dispatch(fetchSaleProducts()); // ✅ Fetch sale products from API
  }, [dispatch]);
